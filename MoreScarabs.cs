...
namespace MoreScarabs
{
    ...
    public class MoreScarabPatches
    {
        ...
        [HarmonyPatch(typeof(BattleMatch.Scarab), "NextturnContinue")]
        public static void NextTurnContinuePrefix(MatchManager __instance)
        {
            ...
            int scarabType = onlyJade ? 2 : MatchManager.Instance.Random.GetRandomIntRange(0, 4);
            ...
            LogDebug("Is it a valid combat to summon a scarab: " + isValidCombat);
            if (isValidCombat)
            {
                int i = MatchManager.Instance.TeamManager.GetNPCAvailablePosition(1);
                if (i > -1)
                {
                    int randomIntRange = MatchManager.Instance.Random.GetRandomIntRange(0, 100);
                    if (randomIntRange < spawnChance)
                    {
                        ...
                        {
                            LogDebug("scarabData found for scarab => " + scarabSpawned + str + " spawning scarab");
                            MatchManager.Instance.TeamManager.CreateNPC(npc, _position: i);
                            // CreateNPCReversePatch(npc, _position: i);
                            Globals.Instance.WaitForSeconds(0.5f);
                            if (scarabType != 3)
                                TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("luckyscarab"), 1);
                            TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("sight"), 3);
                            if (scarabType == 1)
                            {
                                if (AtOManager.Instance.GetNgPlus() == 0)
                                {
                                    if (AtOManager.Instance.GetTownTier() == 1)
                                        TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("zeal"), 5);
                                    else
                                        TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("zeal"), 7);
                                }
                                else if (AtOManager.Instance.GetTownTier() == 1)
                                    TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("zeal"), 6);
                                else
                                    TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("zeal"), 7);
                            }
                            else if (scarabType == 2)
                            {
                                if (AtOManager.Instance.GetNgPlus() == 0)
                                {
                                    if (AtOManager.Instance.GetTownTier() == 1)
                                    {
                                        TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("evasion"), 5);
                                        TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("buffer"), 5);
                                    }
                                    else
                                    {
                                        TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("evasion"), 7);
                                        TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("buffer"), 7);
                                    }
                                }
                                else if (AtOManager.Instance.GetTownTier() == 1)
                                {
                                    TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("evasion"), 6);
                                    TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("buffer"), 6);
                                }
                                else
                                {
                                    TeamNPC[i].SetAuraCurse((Character)null, Globals.Instance.GetAuraCurseData("evasion"), 7);
                            ...
                            LogDebug("Attempting to set traverses for scarab spawn");
